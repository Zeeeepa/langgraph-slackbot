"""
Prompt templates for the Multi-Threaded Agentic Slackbot.

This module provides various prompt templates used by the agents for LLM interactions.
"""
...
ROUTE_PROMPT = ChatPromptTemplate.from_messages([
    ("system", """あなたはユーザーの質問をベクタストアまたはウェブ検索にルーティングする専門家です。
ベクタストアにはエージェント、プロンプトエンジニアリング、アドバーサリアルアタックに関連する文書が含まれています。
これらのトピックに関する質問にはベクタストアを使用し、それ以外の場合はウェブ検索を使用してください。"""),
    ("human", "{question}")
])
